            ...
            if (indexService == null) {
                ...
                for (String type : typesToIntroduce) {
                    ...
                    if (indexMetaData.mappings().containsKey(type)) {
                        ...
                        indexService.mapperService().merge(type, indexMetaData.mappings().get(type).source(), false, "master_refresh_update");
                    }
                }
            }
        ...
        for (MappingTask task : tasks) {
            if (task instanceof RefreshTask) {
                ...
                try {
                    ...
                    DocumentMapper updatedMapper = indexService.mapperService().merge(type, mappingSource, false, "master_refresh_update");
                    processedRefreshes.add(type);
                ...
                try {
                    ...
                    for (String index : request.indices()) {
                        ...
                        if (indexMetaData.mappings().containsKey(MapperService.DEFAULT_MAPPING)) {
                            indexService.mapperService().merge(MapperService.DEFAULT_MAPPING, indexMetaData.mappings().get(MapperService.DEFAULT_MAPPING).source(), false, "put_mapping_api");
                        }
                        // only add the current relevant mapping (if exists)
                        if (indexMetaData.mappings().containsKey(request.type())) {
                            indexService.mapperService().merge(request.type(), indexMetaData.mappings().get(request.type()).source(), false, "put_mapping_api");
                        }
                    }
                    ...
                    for (Map.Entry<String, DocumentMapper> entry : newMappers.entrySet()) {
                        ...
                        DocumentMapper mergedMapper = indexService.mapperService().merge(newMapper.type(), newMapper.mappingSource(), false, "put_mapping_api");
