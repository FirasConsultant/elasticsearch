...
public class GatewayAllocator extends AbstractComponent {
    ...
    public boolean allocateUnassigned(RoutingAllocation allocation) {
        ...
        while (unassignedIterator.hasNext()) {
            ...
            ActivityLevel activityLevel = ActivityLevel.DEFAULT;
            // if we restore from a repository one copy is more then enough
            if (shard.restoreSource() != null) {
                activityLevel = ActivityLevel.ONE;
            }  else {
                try {
                    activityLevel = ActivityLevel.fromString(indexMetaData.settings().get(INDEX_RECOVERY_INITIAL_SHARDS, settings.get(INDEX_RECOVERY_INITIAL_SHARDS, this.initialShards)));
                } catch (Throwable e) {
                    logger.warn("failed to parse activity level", e);
                }
            }

            // not enough found for this shard, continue...
            if (activityLevel.isMet(indexMetaData.getNumberOfReplicas() + 1, numberOfAllocationsFound) == false) {
                // if we are restoring this shard we still can allocate
                if (shard.restoreSource() == null) {
                    // we can't really allocate, so ignore it and continue
                    unassignedIterator.remove();
                    routingNodes.ignoredUnassigned().add(shard);
                    logger.debug("[{}][{}]: not allocating, number_of_allocated_shards_found [{}], total [{}], level [{}]", shard.index(), shard.id(), numberOfAllocationsFound, indexMetaData.getNumberOfReplicas() + 1, activityLevel);
                } else {
